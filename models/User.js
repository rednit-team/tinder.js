...
class User {
	...
	constructor(user) {
		...
		if (user.spotify) this.spotify = {
			connected: user.spotify.spotify_connected,
			spotifyTopArtists: user.spotify.spotify_top_artists?.map((artist) => new TopArtist(artist)),
			spotifyThemeTrack: user.spotify.spotify_theme_track ? new Track(user.spotify.spotify_theme_track) : undefined
		};
